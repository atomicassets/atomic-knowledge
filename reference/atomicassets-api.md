...
Source: `atomicassets-api package.json` (`name`, `repository.url`), `atomicassets-api src/api/server.ts` (`DocumentationServer`, deployed OpenAPI title `EOSIO Contract API`)
...
The atomicassets-api CI workflow triggers on pushes to `main` and on pull requests targeting `main` only. A pull request opened against any other branch (for example a long-lived feature integration branch) starts no checks at all, so an all-green PR page does not mean the change was tested. On an integration branch, verify locally: run a PostgreSQL container, export `POSTGRES_TEST_HOST`, `POSTGRES_TEST_PORT`, `POSTGRES_TEST_USER`, `POSTGRES_TEST_PASSWORD`, and `POSTGRES_TEST_DATABASE` (with `PGSSLMODE=disable` if the container has no TLS), then run `pnpm test:integration:ci` alongside the lint, typecheck, build, and unit-test scripts that the CI workflow runs.
