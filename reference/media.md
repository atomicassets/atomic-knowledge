...
How an Atomic asset points at its media, and what an integrator must do to render it. The chain stores no image bytes and no media column: media is carried as ordinary schema attributes whose values are IPFS references, under field names that are collection convention rather than contract rule. This page covers those conventions and how to resolve them. The wire encoding of the `ipfs` and `image` FORMAT types is in [AtomicAssets attribute serialization](atomicassets/serialization.md); the V2 `FORMAT_TYPE` media-type mechanism is in [AtomicAssets attribute type system](atomicassets/custom-types.md); which data layer a given attribute comes from is in [Attribute data precedence](atomicassets/data-precedence.md). This file does not restate those.
