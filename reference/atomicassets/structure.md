...
Source: atomicassets-contract include/atomicassets.hpp:348-359 (collections table), :14 (`AUTHOR_SWAP_TIME_DELTA`), :13 (`MAX_MARKET_FEE`); src/atomicassets.cpp:91-158 (createcol naming and validation, 24-account caps at :127-128), :194-217 (addcolauth), :254-279 (addnotifyacc, allow_notify gate), :357-442 (author swap actions).
...
Source: atomicassets-contract include/atomicdata.hpp:35-44 (`FORMAT`, `FORMAT_TYPE`); include/checkformat.hpp:32-97 (`check_format`, mandatory name/string line at :95-96, uniqueness at :89-90); include/atomicassets.hpp:363-379 (schemas and schema_types tables); src/atomicassets.cpp:450-475 (createschema), :482-506 (extendschema, append-only), :514-560 (setschematyp).
...
Source: atomicassets-contract include/atomicassets.hpp:383-405 (templates and template_mutables tables), :456 (template_counter default); src/atomicassets.cpp:566-594 (createtempl / createtempl2), :600-624 (deltemplate), :631-653 (locktemplate), :661-689 (redtemplmax), :846-907 (settempldata, emplace/modify/erase branching), :1579-1655 (internal_create_template).
...
Changed in V2: RAM-payer reassignment. Two actions let an asset's `ram_payer` be reassigned without a transfer. `setrampayer(new_payer, asset_id)` requires `new_payer` to already own the asset and sets `ram_payer` to `new_payer` (failing if it is already the payer). `setlastpayer(owner, collection_name)` reassigns `ram_payer` to `owner` for the single highest-`asset_id` row currently in `owner`'s scope (`--owner_assets.end()`, the multi-index's last entry by primary-key order, not necessarily the most recently acquired asset), asserting that asset's `collection_name` matches the argument. Both send the inline `logrampayer` notification. This mechanism shipped in place of an earlier, abandoned custodial-rentals design (a `move` action, a `logmove` notification, and a `holders` table) that was descoped before `v2.0.0-rc1` and exists in no tagged V2 release: `move`, `logmove`, and `holders` are not part of V2. V1 has neither these reassignment actions nor the abandoned holder concept.

Source: atomicassets-contract include/atomicassets.hpp:409-421 (assets table), :455 (asset_counter default); src/atomicassets.cpp:914-940 (setrampayer), :943-976 (setlastpayer), :1535-1547 (logrampayer), :697-788 (mintasset), :796-836 (setassetdata), :1079-1087 (backasset deprecated).
...
- **Notify accounts** (`collections.notify_accounts`, capped at 24, gated by `allow_notify`) - not authorization at all, but a `require_recipient` notification list: every relevant action for the collection will attempt to notify these accounts' smart contracts, so a notify account with a throwing `on_notify` handler can block the action. The 24-account cap on both lists exists partly to bound the low-level partial-row read the contract uses to check membership without deserializing a collection's full (and potentially multi-kilobyte) `serialized_data`.

Source: atomicassets-contract src/atomicassets.cpp:167-442 (author-only and auth-gated actions), :1823-1888 (`partial_read_collection`, `check_has_collection_auth`, `notify_collection_accounts`); 24-account caps at :127-128 (createcol), :212 (addcolauth), :274 (addnotifyacc).
...
Source: atomicassets-contract include/atomicassets.hpp:337-489 (table and table-fetch definitions).
